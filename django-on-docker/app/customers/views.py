...
class register(CreateView):
    ...
    def form_valid(self, form):
        user = form.save()
        messages.success(
            self.request, f'{"Your account has now been created! You are now logged in."}')
...
def c_role(request):
    ...
    if user.is_customer is False:
        if user.is_vendor is False:
            user.is_customer = True
            user.save()
            customer = Customer.objects.create(user=user, email=user.email)
            customer.save()
            messages.success(
                request, f'{"Your account has now been created!"}')
            return redirect('homepage')
    else:
        logout(request)
...
def c_check(request):
    ...
    elif user.is_customer is True:
        return redirect('homepage')
    else:
        logout(request)
        messages.error(
            request, f'{"You are not registered as Customer!Log in Again."}')
    ...
    for product in c_items:
        for p in cart_obj:
            if product.cart == p:
                count += product.quantity
                cartitems.append(product)
    context = {'cartitems': cartitems, 'cart_obj': cart_obj, 'count': count,
               'obj': Cart.objects.get(user=request.user, is_paid=False)}
...
def success(request):
    ...
    if request.user.amount >= price:
            ...
            for p in cart_obj:
                if product.cart == p:
                    ...
                    template1 = render_to_string(
                        'customers/vendor_email.html', {'product': product, 'obj': obj})
                    email_vendor = EmailMessage('Shopmart.in purchase made', template1, settings.EMAIL_HOST_USER, [
                                                product.item.vendor.email])
                    email_vendor.fail_silently = False
                    email_vendor.send()
                    product.item.quantity_purchased += product.quantity
                    product.item.save()
                    order = MyOrder.objects.create(customer=request.user, vendor=product.item.vendor.username, image=product.item.image,
                                                   title=product.item.title, description=product.item.description, price=product.item.price, quantity=product.quantity)
                    order.save()

        template = render_to_string(
            'customers/customer_email.html', {'obj': obj})
        email_customer = EmailMessage(
            'Your Shopmart.in order', template, settings.EMAIL_HOST_USER, [request.user.email])
        ...
        messages.error(
            request, f'{"Insufficient Balance in Account! Update your Account."}')
