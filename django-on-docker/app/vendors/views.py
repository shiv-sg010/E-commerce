...
class ItemDeleteView(UserPassesTestMixin, DeleteView):
    ...
    def get_success_url(self):
        messages.success(
            self.request, f'{"Your item is successfully deleted."}')
...
def v_check(request):
    ...
    elif user.is_vendor is True:
        return redirect('dashboard', user.username)
    else:
        logout(request)
        messages.error(
            request, f'{"You are not authorized as Vendor! Log in Again."}')
...
def export_orders_xls(request):
    ...
    rows = MyOrder.objects.filter(vendor=request.user).values_list(
        'vendor', 'title', 'quantity', 'price')
    for row in rows:
        row_num += 1
